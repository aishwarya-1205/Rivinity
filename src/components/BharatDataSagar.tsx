...
const BharatDataSagar = () => {
    ...
    return (
        <section ref={sectionRef} className="py-32 bg-transparent transition-colors duration-300 relative overflow-hidden">
            ...
            <div className="absolute inset-0 opacity-[0.08] dark:opacity-[0.15] pointer-events-none overflow-hidden">
                <svg className="w-full h-full" viewBox="0 0 1440 800" preserveAspectRatio="none">
                    ...
                    <path
                        ...
                        opacity="0.6"
                    >
                        <animate attributeName="stroke-dashoffset" from="0" to="1000" dur="25s" repeatCount="indefinite" />
                    </path>
                    <defs>
                        <linearGradient id="gradient-stream" x1="0%" y1="0%" x2="100%" y2="0%">
                            <stop offset="0%" stopColor="#3b82f6" stopOpacity="0" />
                            <stop offset="50%" stopColor="#3b82f6" stopOpacity="1" />
                            <stop offset="100%" stopColor="#06b6d4" stopOpacity="0" />
                        </linearGradient>
                        <linearGradient id="gradient-stream-2" x1="0%" y1="0%" x2="100%" y2="0%">
            ...
            <div className="container mx-auto px-6 relative z-10">
                <div className="flex flex-col lg:flex-row items-center gap-16">
                    {/* Text Content */}
                    <div className={`lg:w-1/2 text-left transition-all duration-1000 transform ${isVisible ? 'translate-x-0 opacity-100' : '-translate-x-20 opacity-0'}`}>
                        ...
                        <h2 className="text-5xl md:text-6xl font-display font-bold text-slate-900 dark:text-white mb-6 leading-tight">
                            Bharat <br />
                        ...
                        <p className="text-xl text-slate-600 dark:text-slate-300 leading-relaxed max-w-lg mb-8">
                            The world's largest repository of Indian language data. We are digitizing India's heritage—creating an ocean of intelligence for the AI age.
                        </p>
                    ...
                    <div
                        className={`lg:w-1/2 relative h-[500px] w-full flex items-center justify-center transition-all duration-1000 delay-300 transform ${isVisible ? 'translate-x-0 opacity-100' : 'translate-x-20 opacity-0'}`}

                    >
                        {/* Central Core */}
                        <div className="absolute w-32 h-32 bg-gradient-to-br from-blue-600 to-cyan-400 rounded-full blur-[60px] opacity-40 animate-pulse-slow"></div>
                        <div className="relative z-20 w-40 h-40 bg-white/10 dark:bg-slate-900/40 backdrop-blur-xl rounded-full border border-blue-500/30 flex items-center justify-center shadow-[0_0_40px_rgba(59,130,246,0.2)]">
                            <Globe size={64} className="text-blue-500 opacity-80" strokeWidth={1} />
                        ...
                        >
                            ...
                            <div className="absolute w-[450px] h-[180px] rounded-full border border-blue-500/10 dark:border-white/5"></div>
...
const OrbitingBadges = () => { // Removed isHovering prop
    ...
    return (
        <>
            {badges.map((card, idx) => {
                ...
                // Static calculation
                const x = Math.cos(radianOffset) * 225;
                const y = Math.sin(radianOffset) * 90;
                ...
                return (
                    <motion.div
                        ...
                        style={{ x, y }}
                    >
                        <div className="group relative p-4 bg-white/90 dark:bg-slate-900/90 backdrop-blur-xl rounded-2xl border border-slate-200 dark:border-slate-700 shadow-xl flex items-center gap-3 transition-all duration-300 hover:scale-110 hover:shadow-blue-500/30 hover:border-blue-500/50 cursor-pointer min-w-[160px] -translate-x-1/2 -translate-y-1/2">
                            <div className={`w-10 h-10 rounded-full ${card.color} flex items-center justify-center text-white shadow-lg group-hover:animate-pulse`}>
                                <card.icon size={20} />
                            </div>
                            <div>
                                <p className="font-bold text-slate-900 dark:text-white text-sm">{card.label}</p>
                                <p className="text-[10px] text-slate-500 font-mono">1.2 PB Live</p>
                            </div>
                        </div>
                    </motion.div>
                );
